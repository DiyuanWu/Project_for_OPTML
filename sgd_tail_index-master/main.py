...
if __name__ == '__main__':
    ...
    parser.add_argument('--close_check_lb', default=10000, type=int)
    parser.add_argument('--close_check_hb', default=10000, type=int)
    parser.add_argument('--close_check_freq', default=100, type=int)
    parser.add_argument('--eval_Hessian_freq', default=10000, type=int)
    ...
    for i, (x, y) in enumerate(circ_train_loader):
        ...
        if (i % args.eval_freq == 0) or \
          (i>=args.close_check_lb and i<=args.close_check_hb and i%args.close_check_freq==0):
        ...
        if i % args.eval_Hessian_freq == 0:
            print('checking Hessian')
            # check the Hessian
            # create the hessian computation module
            hessian_comp = hessian(net, crit, data=next(iter(train_loader)), cuda=False)
            density_eigen, density_weight = hessian_comp.density()
            if not os.path.exists(args.save_dir):
                os.makedirs(args.save_dir)
            torch.save([density_eigen, density_weight], args.save_dir + '/eigenvalues_step%d.pyT'%i)


            
        if (i % args.eval_freq == 0) or STOP:
            if not os.path.exists(args.save_dir):
                os.makedirs(args.save_dir)
            else:
